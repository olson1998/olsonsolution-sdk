...
import com.olsonsolution.common.spring.application.config.context.LocalContextManagerConfig;
import com.olsonsolution.common.spring.application.config.time.TimeUtilsConfig;
import com.olsonsolution.common.spring.application.datasource.TenantDataSourceSpecManagingConfig;
...
@ContextConfiguration(classes = {
        ...
        LocalContextManagerConfig.class,
        TenantDataSourceSpecManagingConfig.class,
        AsyncConfig.class,
        JpaConfig.class,
...
public abstract class SpringApplicationJpaTestBase implements InitializingBean {
    ...
    protected static final Set<String> TENANTS = Set.of(
            "SQLSERVER_TENANT", "POSTGRES_TENANT", "MARIADB_TENANT"
    );
    ...
    public void afterPropertiesSet() throws Exception {
        TENANTS.stream().map(db -> db + "_DataSource").map(dataSourceName -> {
            JpaDataSourceSpec jpaDataSourceSpec = DomainJpaSpecDataSource.builder()
                    .jpaSpec(SYSTEM_JPA_SPEC)
                    .dataSourceName(dataSourceName)
                    .permission(RWX)
                    .build();
            SqlDataSource sqlDataSource = sqlDataSourceProvider.findDestination(jpaDataSourceSpec).orElseThrow();
            DataSource dataSource = sqlDataSourceFactory.fabricate(sqlDataSource, RWX);
            return migrationService.migrateAsync(dataSource);
        }).collect(Collectors.collectingAndThen(
                Collectors.toList(),
                migs -> CompletableFuture.allOf(migs.toArray(CompletableFuture[]::new))
        )).join();
    }
    ...
    static void setDataSourceProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add(prefix + ".0.name", () -> "SQLSERVER_TENANT_DataSource");
        ...
        registry.add(prefix + ".1.name", () -> "POSTGRES_TENANT_DataSource");
        ...
        registry.add(prefix + ".2.name", () -> "MARIADB_TENANT_DataSource");
